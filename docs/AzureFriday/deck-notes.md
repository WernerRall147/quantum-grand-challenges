...
| **Evaluation latency** | **22s median, and you can quote it** | 40 probe runs 12-18 Sep 2026: min 15.4s, median 21.9s, p95 28.7s, max 33.8s. Ten live calls on 18 Sep across five prompts: 18.7-24.4s. Earlier guidance said not to quote a number, on the strength of five hand-run calls on 2026-08-31 that included a 98.0s outlier; that prompt now returns in 18.7s and the spread has not recurred in 40 automated samples. Still rehearse against two minutes - the router picks the model |
| **~92% of that time is model inference** | the answer to "where does the time go?" | `model_seconds` on 18 Sep: 16.5-22.4s of an 18.7-24.4s total. Everything this repo owns - routing, retrieval, assembly - is 1.0-2.2s, network round trip included |
...
| **657 tests passing** | kept current by `tooling/test_doc_claims.py` | `pytest -q`. This line was 116 when the suite was 214, and 214 when it was 243 |
| **The router decides in well under a millisecond** | beat 2's line, and the one figure worth saying out loud | Production 2026-09-01: `route_platform` 0.1 ms in requests whose model call took 21.5 s and 25.1 s. Re-measured in-process 18 Sep over the 22 evaluation cases: median 0.024 ms, max 0.095 ms. Say it as a ratio - "a fraction of a millisecond against twenty-odd seconds" - because the model call has been seen at 98s and the ratio survives that, an exact figure does not |
