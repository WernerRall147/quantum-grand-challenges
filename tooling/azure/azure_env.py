"""Shared Azure environment helpers for problem-level workflows."""
...
class AzureEnvError(ValueError):
    """Raised when Azure environment values are missing or invalid."""
...
def load_env_file(path: Path) -> Dict[str, str]:
    if not path.exists():
        raise FileNotFoundError(
            f"Azure env file not found: {path}. Copy tooling/azure/.env.azure.example to your local env file and fill it in."
        )
...
def load_azure_env(env_file: Path) -> Dict[str, str]:
    ...
    if missing:
        raise AzureEnvError("Missing required Azure env keys: " + ", ".join(missing))

    placeholders = [key for key in REQUIRED_KEYS if _is_placeholder(merged[key])]
    if placeholders:
        raise AzureEnvError("Azure env contains placeholder values for: " + ", ".join(placeholders))
