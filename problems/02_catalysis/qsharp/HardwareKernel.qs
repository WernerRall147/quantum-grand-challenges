...
// Problem: 02_catalysis (QPE for the H2 ground-state energy)
// Target profile: Adaptive_RI
//
// Two phase qubits of quantum phase estimation (QPE) for the Hamiltonian that
// Main.MolecularQPE uses, with one Trotter step per U so it fits current hardware.
...
operation CatalysisQPEKernel() : Result[] {
    // H2, STO-3G, 0.735 angstrom, parity mapping with two-qubit reduction.
