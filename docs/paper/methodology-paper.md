...
> energy; the quantum-walk kernel's position never depended on its coin; and Shor's order
> finding did not find the period, because its modular multiplier was right for two of
> its bases and its inverse QFT was wrong, so the demonstration Section 7.2 said factored
> 15 usually did not, and its resource estimate used a base that shares a factor with 15
> and so contained no modular arithmetic. Those kernels were rewritten and tested against
> exact simulation (Section 4.2), the resource estimates now target the rewritten routines
> (Section 4.4), every calibration ensemble was regenerated, and CI now fails when the
> ensemble behind a Stage C or D label does not match the sources it claims to describe
> (Section 5.2). Section 9 also records that Part 6 of the lecture series [15], on cost,
> has been published; it had been listed as forthcoming.

> **Revision, 2026-09-26, after the v3.2.0 archive.** A second audit compared the remaining kernels with exact
> simulation and their documentation with their code. Canonical amplitude estimation in the
> risk-analysis problem reflected about A†|0⟩ instead of A|0⟩ and omitted the −1 that gives the
> controlled Grover iterate its eigenphases ±2θ, so its phase register peaked at the wrong
> outcomes; its one sanity test used a self-inverse state preparation at a = 1/2, where neither
> fault shows. Its published tail probability matched no computation of the model (the circuit
> encodes 16.14%), its ensembles came from the faulty kernel, its calibration read the
> phase register in the wrong bit order, and its IQAE driver was not the algorithm of Grinko et
> al. [22] that it cited. The kernel now follows Theorem 11 of Brassard et al. [21] exactly, the
> driver implements Grinko et al.'s Algorithm 1 and its intervals hold their stated confidence
> over repeated runs, and IQAE is compared with Monte Carlo on the same distribution at equal
> precision (Section 7.2). The high-frequency-trading problem, listed as amplitude estimation for
> Value at Risk, never implemented amplitude estimation: it samples a marked-state probability
> directly. The two HHL kernels were not HHL and have been rebuilt as the textbook circuit [23],
> which reproduces A⁻¹b exactly for the 2×2 diffusion matrix and within its clock resolution for
> the other system. The QAOA calibrations and estimates ran fixed angles (an expected cut of 0.13
> on a triangle, worse than a random cut) and now use grid-searched angles on the models the
> demonstrations build; the protein-folding kernel is a four-variable toy QUBO and is now
> described as one, and three QAOA hardware kernels (08, 12, 20) implemented other models than
> the programs they stood for. The swap-test
> encoding dropped signs and its hardware kernel compared unrelated states. Section 4.3's
> statement that every classical baseline is the standard approach for its domain did not hold
> for eight problems. Resource estimates now count Toffoli gates, which were previously dropped
> (10,687 in the risk-analysis kernel), and Lesson 3 is recomputed; the estimator summaries of
> the archived problems had printed a mock constant as their estimate. Under the pinned QDK one
> hardware kernel no longer compiled to QIR, and CI now compiles all twenty. The noise study in
> Section 7.2 was regenerated for the current kernels; the emulator histograms still describe
> the kernels of April 2026. Counting both revisions, 16 of the 20 kernels (01, 02, 03, 04, 06,
> 07, 08, 09, 11, 12, 13, 14, 17, 18, 19, 20) either did not compute
> what they were described as computing or were mislabelled, although every one of them compiled
> and passed CI.
...
- **CI/CD:** Eight GitHub Actions workflows, three of which run on pull requests (Section 5.1)
...
| HHL | 04, 13 | 5-7 | Controlled exp(iAt), QPE, controlled Ry(2 arcsin(C/λ)), inverse QPE, post-selection |
| Amplitude estimation† | 03 | 5-14 | State prep, oracle, Grover^k; canonical QAE adds a QPE precision register, IQAE does not |
...
| Trotter | 19 | 2-4 | ZZ couplings, transverse field (an Ising chain) |

\* The VQE routines (2-3 qubits) remain in each `Main.qs`. On 2026-04-18 (#66) the hardware kernels of these five problems were replaced by quantum phase estimation (QPE) kernels, but every controlled evolution in them was built from Rz and CNOT-conjugated Rz gates, a diagonal unitary: from a computational basis state the phase register returned that state's diagonal energy, never the ground-state energy. On 2026-09-25 they were rewritten as Trotterized QPE on a 4-qubit Jordan-Wigner two-site Hubbard model (01), the parity-reduced 2-qubit STO-3G H₂ Hamiltonian (02), the 2-qubit deuteron model of Dumitrescu et al. [20] (17), and 2-qubit models labelled as toys (07, 14), and `tooling/test_qpe_kernels.py` now compares each against exact diagonalization of the same Hamiltonian. The hardware kernels use 2 phase bits (4-6 qubits) and the resource estimates 10 (12-14 qubits). The emulator histograms and the noise study in Section 7.2 were produced by the VQE kernels and predate both changes; the calibration ensembles and resource estimates were regenerated from the rewritten QPE routines. The same revision found that the quantum-walk kernel (18) swapped its position qubits whichever way the coin pointed, and rewrote its shift as a coin-controlled step (`tooling/test_exciton_walk.py`). It also found that Shor's order finding (09) did not find the period. Its mod-15 multiplier was right for a = 2 and 4 only, its inverse QFT processed the qubits in the wrong order with angles twice too large, and its hardware kernel applied one multiplication controlled by the wrong qubit. Its resource estimate ran base 3, which shares a factor with 15, so the multiplier applied nothing and the estimate contained no modular arithmetic. All four were corrected and are tested against the exact period of every base coprime to 15 (`tooling/test_shor_kernel.py`); the estimate now runs base 7. The Trotter kernel (19) was found to compute what its code says but not what it was called: it evolves a transverse-field Ising chain, which has no gauge fields and maps to free fermions, and the quantity described as a Wilson loop is the product of Z over all sites. It is now described as a spin-chain stand-in and checked against exact evolution (`tooling/test_ising_chain_kernel.py`).

On 2026-09-26 the remaining kernels were compared with exact simulation. Canonical QAE (03) built its reflection as A†S₀A instead of AS₀A† and omitted the −1 in Q = −AS₀A†S_χ, which becomes a relative phase once Q is controlled, so its phase register peaked at 0 and 32 of 64 where Theorem 11 of Brassard et al. [21] puts the peaks at 8 and 56; `tooling/test_qae_kernel.py` now compares the register with that theorem from the state vector, and the IQAE driver implements Algorithm 1 of Grinko et al. [22]. The HHL kernels (04, 13) used ad hoc rotations and a per-bit inversion; they are now the textbook circuit [23], with exact controlled evolution and a C/λ inversion, and `tooling/test_hhl_kernel.py` ties their outputs to a direct solution of Ax = b. The swap-test encoding (11) dropped the signs of its amplitudes, and three QAOA hardware kernels implemented other models than the programs they stood for: 08 and 12 ran chains with equal couplings instead of their weighted QUBOs, and 20 used different biases and two of its three couplings (`tooling/test_swap_test_kernel.py`, `tooling/test_qaoa_kernels.py`).

† Problem 06 was listed in this row until 2026-09-26. Its kernel prepares a 2-qubit state and measures a marker once per shot, sampling a marked-state probability directly; it never implemented amplitude estimation, so it is not counted as a family.

### 4.3 Classical Baselines

Every problem includes a deterministic classical baseline, and all baselines produce JSON output conforming to a shared schema. Each was meant to be both the standard classical approach for its domain and the comparison target for its quantum kernel, as exact diagonalization is for the QPE models and brute-force enumeration is for MaxCut at small scale. For eight problems it is not both. The trading baseline (06) is a moving-average strategy unrelated to the kernel's marked-state probability; the protein-folding (08) and mission-planning (20) baselines are heuristic scoring scripts and the scheduling baseline (12) a greedy tardiness heuristic, none of which evaluates its kernel's QUBO; the climate baseline (13) is an energy-balance model of a different system; the machine-learning baseline (11) is kernel ridge classification, a different task from the swap test's overlap estimate; the cryptography baseline (10) is a toy lattice-attack cost formula, while the key-search kernel's comparator is exhaustive search; and the Trotter baseline (19) is a heuristic formula rather than the exact solution of the Ising chain, which is classically easy. The risk-analysis Monte Carlo sampled the continuous model rather than the circuit's 16-level discretization until a comparison on the same distribution was added in September 2026. Until then this section described every baseline as the standard approach for its domain; each README now says what its baseline computes.

### 4.4 Resource Estimation Pipeline

We employ a centralized estimation manager (`tooling/generate_estimates.py`) that runs the Quantum Resource Estimator v3 (`qdk.qre`) [19] on each problem's estimation entry point. All 20 problems have real profiles targeting a gate-based superconducting architecture (50 ns gates, 10⁻³ error rate) with a surface code, round-based magic-state factories and a total error budget of 10⁻³. QRE v3 returns a Pareto frontier trading physical qubits against runtime; we report the point with the fewest physical qubits, which is also the slowest. The estimated programs are the toy instances themselves, compiled for fault-tolerant execution, not utility-scale versions of the problems; for the five QPE problems of Table 1 they are the QPE routines with a 10-bit phase register. Physical qubit counts range from 1,746 (3-qubit QEC repetition code) to 754,715 (QPE on the materials-discovery model). The pipeline also supports batch calibration via `tooling/generate_calibration_ensemble.py`, which runs each kernel 20 times to produce ensemble statistics with 95% confidence intervals (for the QPE problems each run is the most frequent of 8 estimates at 8 phase bits). The intervals are normal approximations: for four problems (02, 05, 16 and 17) every run returned the same value, the interval has zero width and does not bound the mean, and the site reports it as undefined. All 20 committed ensembles now carry valid statistics and record hashes of the Q# sources they ran; before the 2026-09-25 revision 10 of the 20 did (Section 8, limitation 8). Until 2026-09-26 the estimates reported no Toffoli gates at all: the trace reports them as CCX, which the summary did not map, so the risk-analysis kernel's 10,687 Toffolis went uncounted.
...
Five further workflows deploy the website and the evaluator API, run a nightly resource-estimation sweep, KPI refresh and runnable-correctness audit, schedule an uptime probe of the deployed API (its cron asks for every 30 minutes; GitHub's throttling of scheduled workflows has produced roughly one run every four hours), and, since September 2026, run a weekly check that the deployed evaluator's Q# generation returns programs that compile and do quantum work. Earlier versions of this paper listed seven pull-request checks, including dedicated Stage D readiness, runnable-correctness and reporting-integrity gates; these were consolidated or removed between April and June 2026 (#67, #124), and the correctness audit now runs nightly rather than per pull request. None of the checks is configured as a required status check on the main branch, so a failing check reports a problem but does not block a merge.
...
| Quantum VaR (direct sampling; not QAE) | Sampling | 3 | ~10 | Succeeded |
...
- Algorithmic correctness at small scale (Grover finds marked items, order finding returns the period of every base coprime to 15, QEC corrects single bit-flips, canonical QAE's phase register follows Theorem 11 of Brassard et al. [21], HHL returns A⁻¹b to within its clock resolution, which each added clock bit halves (exactly for the 2×2 diffusion matrix), and the QPE, quantum-walk, QAOA and swap-test kernels sample the distributions that exact simulation of the same circuits predicts)
...
The H2-1SC syntax checker validates that circuits are well-formed QIR but does not simulate quantum behavior. In April 2026, H2-1E emulator results were collected for all 20 problems (100 shots each), and 19 were also run on the Rigetti QVM. For 14 of those 19, the most frequent outcome is the same unique outcome on both platforms; for two more (05, 08) the QVM histogram has a tie that includes the H2-1E mode; for three (03, 12, 20) the modes differ. The total variation distance between the paired 100-shot histograms ranges from 0.00 to 0.44 (median 0.06). With 100 shots, sampling error alone is up to ±0.05 in each outcome probability (one standard deviation), so these comparisons show that both platforms compiled and executed the same programs; they are not evidence about hardware behavior. They ran the kernels of April 2026: for 01, 02, 07, 14 and 17 the VQE kernels, for 18 a walk whose position never moved, for 09 an order-finding kernel that did not find the period, for 03 a QAE kernel with the wrong Grover iterate, for 04 and 13 kernels that were not HHL, for 11 a swap test between unrelated states, and for 05, 08, 12 and 20 QAOA kernels that differed from the programs now estimated (Section 4.2).

A separate local simulation with the QDK's depolarizing noise model at strengths p = 0.001, 0.01 and 0.05 (100 shots each) measures the classical fidelity between each kernel's noisy and ideal output histograms. It was regenerated for the current kernels on 2026-09-26; the April 2026 run had used the kernels described above. At p = 0.05 the fidelities range from 0.12 (Grover database search with three iterations) to 1.00 (the QEC kernel, whose output is its corrected bit), with the five QPE kernels at 0.39-0.84, Shor at 0.53 and the quantum walk at 0.49. These estimates are shot-noise limited: several rise between p = 0.001 and p = 0.01, which added noise cannot cause. A September 2026 run of the Grover database-search kernel (four qubits, three iterations) on H2-1E returned the marked item in 80 of 100 shots, against an ideal 96.1% and about 6 in 100 for random guessing (`problems/archived/15_database_search/azure_runs/`).
...
- **Shor (09_factorization)**: Order finding for N = 15 returns s × 16 / r for every base coprime to 15, and the classical step recovers 15 = 3 × 5. Earlier versions of this paper said the implementation correctly factored 15; it did not. The counting register was nearly uniform, and in three runs of the demonstration the most frequent candidate period was 8 and no factor was found in 8 of 9 attempts (Section 4.2). The controlled modular multipliers are fixed permutations compiled for N = 15, so the circuit demonstrates the structure of period finding, not scalable modular arithmetic; compiled demonstrations of this kind say little about factoring in general [16]. It is a textbook demonstration, not a research contribution
- **QAE (03_qae_risk)**: the canonical kernel's phase register matches Theorem 11 of Brassard et al. [21] to 10⁻⁹. IQAE on the kernel returned [0.152, 0.191] around the encoded a = 0.1614 from 600 applications of the state preparation, where Monte Carlo on the same distribution needs 1,373 samples for the same half-width; at half-width 0.0004 the counts are 42,715 and 3.2 million. These are noiseless query counts, not run times
- **HHL (04_linear_solvers, 13_climate_modeling)**: on the 2×2 diffusion matrix the post-selected output equals |A⁻¹b|² = [0.8, 0.2] with success probability 5/9, the textbook values for C = 1; for the other system, whose eigenvalues fall between clock values, the fidelity with A⁻¹b rises from 0.99957 with 3 clock bits to 0.99998 with 4 and 0.99999 with 5
...
3. **Mock estimates previously created false confidence**: Uniform mock resource estimates across all problems suggested comparable resource requirements. Real Quantum Resource Estimator profiles now reveal a range of about 430×: from 1,746 physical qubits (QEC repetition code) to 754,715 (QPE on the materials-discovery model). Explicit T-gate counts are small (QAE: 15, HHL: 28 and 40, Shor: 9), but that does not mean the kernels avoid magic-state distillation. Every arbitrary-angle rotation must be synthesized from T states and every Toffoli consumes a CCZ state, so the rotation-only QAOA kernels devote 71-74% of their physical qubits to T factories, the rotation-dominated QPE estimates 90-94%, and the QAE kernel's cost is set by its 3,713 rotations and 10,687 Toffolis rather than its 15 T gates.
...
6. **A test must be able to fail at its own parameters**: The QAE kernel's only sanity test prepared a uniform superposition with H, which is its own inverse, at an amplitude of 1/2, where the reversed reflection and the missing sign both leave the output unchanged; it passed for months. The Trotter export used angles so small that a doubled coupling passed, and a fixed simulator seed makes every shot in a run identical, which produced a confident IQAE interval that excluded the true amplitude. Each new check in this revision was run against the old code, or against a deliberately broken variant, and seen to fail before it was trusted.
...
3. **Classical baselines are deliberately weak.** We use textbook algorithms (brute-force, naive Monte Carlo) for cross-domain standardization, and for eight problems the baseline does not compute the quantity the quantum kernel estimates (Section 4.3). Any comparative statement must acknowledge that state-of-the-art classical methods would perform far better. Quantum advantage claims have repeatedly weakened or vanished when stronger classical methods were applied [8, 9, 10].
...
5. **Utility-scale screening.** Following Hoefler, Häner and Troyer [11] and Troyer's lecture series on utility-scale quantum computing [15], the project's evaluator applies six filters: a proven speedup (F1), a speedup that survives data input and output (F2), a speedup that survives error-correction overhead (F3), a naturally quantum problem (F4), a feasible crossover problem size (F5) and, for electronic structure, a preparable guiding state (F6) [17]. Eleven of the 20 problems fail at least one filter and are archived (the archival was made in April 2026 with the first five filters; F6 was added in August 2026): four with quadratic speedups (QAE ×2, Grover ×2; the trading problem was archived for the amplitude estimation it was meant to use, although its kernel samples directly), three whose speedup depends on loading classical data (HHL ×2, the swap-test classifier, for which provable kernel separations exist only for constructed datasets [24]), and four QAOA problems with no proven speedup. The nine that remain are quantum simulation problems (the five QPE problems of Table 1, a quantum walk and a Trotter simulation), Shor factoring, and the QEC repetition code, which is retained as infrastructure rather than as an application. The filters were applied to each problem class, not to its toy instance, and for two problems the difference matters. The quantum-walk kernel follows a single exciton, whose exact classical simulation already takes time polynomial in the number of sites, so its pass rests on many-body or strongly coupled open-system transport that the instance does not model. The Trotter kernel is a transverse-field Ising chain, which maps to free fermions and is classically solvable at any size, so its pass rests on interacting lattice gauge dynamics that it does not model either. Passing the filters is a screen, not evidence of advantage: even for ground-state quantum chemistry, a generic exponential quantum advantage has not been established [18], in part because preparing a state with sufficient overlap with the ground state may itself be hard, which is the condition F6 tests.
...
- Re-run the emulator studies for the rewritten QPE, quantum-walk, Shor, QAE, HHL, QAOA and swap-test kernels (Section 8, limitation 8); the noise study was regenerated on 2026-09-26
- Port the risk-analysis ensemble pipeline (`python/analyze.py`), which still calls the retired `dotnet` toolchain, to the `qdk` package, and regenerate its multi-instance ensembles
...
The practical lessons  that placeholders masquerade as implementations, that encoding bugs survive validation, that mock estimates create false confidence, that presence checks let evidence go stale, that a test can be unable to fail at its own parameters  are transferable to any quantum development effort. We hope the framework's insistence on honest self-assessment contributes to healthier practices in the quantum computing community.
...
[21] G. Brassard, P. Høyer, M. Mosca, and A. Tapp, "Quantum amplitude amplification and estimation," Contemporary Mathematics 305, 53-74, 2002 (arXiv:quant-ph/0005055).

[22] D. Grinko, J. Gacon, C. Zoufal, and S. Woerner, "Iterative quantum amplitude estimation," npj Quantum Information 7, 52, 2021 (arXiv:1912.05559).

[23] A. W. Harrow, A. Hassidim, and S. Lloyd, "Quantum algorithm for linear systems of equations," Physical Review Letters 103, 150502, 2009 (arXiv:0811.3171).

[24] Y. Liu, S. Arunachalam, and K. Temme, "A rigorous and robust quantum speed-up in supervised machine learning," Nature Physics 17, 1013-1017, 2021 (arXiv:2010.02174).
...
| 04 | Linear Solvers | HHL | 5-7 | B | Archived |
| 05 | MaxCut | QAOA | 3 | D | Archived |
| 06 | Trading | Direct sampling (planned: QAE) | 3 | B | Archived |
| 07 | Drug Discovery | VQE, then QPE | 2-12 | C | Active |
| 08 | Protein Folding | QAOA (toy QUBO) | 4 | B | Archived |
| 09 | Factorization | Shor | 8 | C | Active |
| 10 | Cryptography | Grover | 3-5 | B | Archived |
| 11 | Machine Learning | Swap Test | 5 | B | Archived |
| 12 | Optimization | QAOA (toy QUBO) | 4 | B | Archived |
| 13 | Climate | HHL (2×2 toy) | 5-7 | B | Archived |
...
| 20 | Space Planning | QAOA (toy QUBO) | 4 | B | Archived |
