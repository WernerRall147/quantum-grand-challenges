...
> descriptions and citations. Section 8 adds a limitation found during this check: five
> problems keep Stage C labels on evidence that predates the rewrite of their kernels,
> and three hold them without valid calibration statistics.
...
|---|---|---|---|
| VQE* | 01, 02, 07, 14, 17 | 2-3 | Ry, CNOT, Rz, Pauli measurement |
...
\* On 2026-04-18 (#66) the hardware kernels of these five problems were replaced by 4-qubit quantum phase estimation (QPE) kernels. Every result reported for them in this paper (emulator histograms, noise study, calibration ensembles and resource estimates) was produced by the VQE circuits: the first three predate the change, and the resource-estimate entry points still invoke the VQE energy routines. No result from the QPE kernels is reported.
...
1. **CI/CD**: its build-and-test job validates workflow syntax, compiles and runs all 20 Q# projects, runs the classical baseline tests and the evaluator test suite, validates JSON schemas, enforces the maturity policy (Section 5.2), and runs the documentation-claim and evidence-provenance guards under `tooling/`. That last step was added on 2026-09-25; until then CI did not run those guards, so the stage-count check cited in the errata above failed only when someone ran it locally
...
On 12-13 April 2026, the hardware kernels of all 20 problems were compiled from Q# to QIR with the modern QDK (qsharp 1.27, the version pinned at the time) and accepted by the Quantinuum H2-1SC syntax checker via Azure Quantum. A syntax checker verifies that a program is well formed for the target; it does not simulate it. Table 2 lists the kernels as recorded then, with approximate gate counts: 20 kernels from 19 problems (two for 01). The 04_linear_solvers kernel also passed and is not listed. The VQE kernels in the table (six rows covering five problems) were replaced by QPE kernels five days later (Section 4.2); the replacements were submitted to the H2-1E emulator on 10 June 2026, but their results were never retrieved into the committed run history.
...
Of the 20 problems, 9 have reached Stage C (declared on the basis of calibration ensembles and real Azure Quantum Resource Estimator profiles; limitation 8 in Section 8 records where that evidence falls short), 8 remain at Stage B, and **3 have reached Stage D** (advantage evidence packages with explicit claim contracts). The 3 Stage D problems (QAE risk analysis, QAOA MaxCut, Grover database search) filed claim contracts in the categories "theoretical" (QAOA MaxCut and QAE risk analysis) and "projected" (Grover database search); the QAE README gives "projected", so the repository's two records of that claim disagree. None claims a demonstrated advantage. The QAOA MaxCut contract states that there is "no proven quantum advantage for MaxCut QAOA at any constant depth" and identifies the Goemans-Williamson 0.878-approximation [7] as the polynomial-time classical competitor. The Grover contract rests on the provably optimal O(√N) query speedup and lists oracle synthesis overhead as its main residual risk. It does not establish a problem size at which the speedup becomes practical, and for unstructured data the cost of loading the database into an oracle or QRAM is itself O(N).
...
- Algorithmic correctness at small scale (Grover finds marked items, Shor factors 15 with a circuit compiled for that modulus, QEC corrects single bit-flips)
...
The H2-1SC syntax checker validates that circuits are well-formed QIR but does not simulate quantum behavior. In April 2026, H2-1E emulator results were collected for all 20 problems (100 shots each), and 19 were also run on the Rigetti QVM. For 14 of those 19, the most frequent outcome is the same unique outcome on both platforms; for two more (05, 08) the QVM histogram has a tie that includes the H2-1E mode; for three (03, 12, 20) the modes differ. The total variation distance between the paired 100-shot histograms ranges from 0.00 to 0.44 (median 0.06). With 100 shots, sampling error alone is up to ±0.05 in each outcome probability (one standard deviation), so these comparisons show that both platforms compiled and executed the same programs; they are not evidence about hardware behavior.

A separate local simulation with the QDK's depolarizing noise model at strengths p = 0.001, 0.01 and 0.05 (100 shots each, April 2026) measured the classical fidelity between each kernel's noisy and ideal output histograms. At p = 0.05 the 2-qubit VQE kernels retained 0.83-0.96, while three kernels fell below 0.40 (Grover database search with three iterations 0.15, quantum walk 0.30, 8-qubit Shor 0.38). These estimates are shot-noise limited: several rise between p = 0.001 and p = 0.01, which added noise cannot cause. A September 2026 run of the Grover database-search kernel (four qubits, three iterations) on H2-1E returned the marked item in 80 of 100 shots, against an ideal 96.1% and about 6 in 100 for random guessing (`problems/archived/15_database_search/azure_runs/`).
...
3. **Mock estimates previously created false confidence**: Uniform mock resource estimates across all problems suggested comparable resource requirements. Real Quantum Resource Estimator profiles now reveal a range of about 210×: from 1,746 physical qubits (QEC repetition code) to 369,400 (canonical QAE risk kernel). Explicit T-gate counts are small (QAE: 15, HHL: 12, Shor: 6), but that does not mean the other kernels avoid magic-state distillation. Every arbitrary-angle rotation must be synthesized from T states, so the rotation-only VQE and QAOA kernels also devote 63-91% of their physical qubits to T factories, and the QAE kernel's cost is driven by its 3,713 rotations rather than its 15 T gates.
...
5. **Presence checks let evidence go stale**: The policy check confirms that a declared stage meets its minimum and that two README sections exist; it never looks at the evidence Stage C requires. Five problems kept Stage C labels after their kernels were rewritten, and three hold them without valid calibration statistics (Section 8, limitation 8), and nothing failed.
...
8. **Stage labels are declared, not verified.** Each README states its own stage, and CI checks only that the declaration meets a per-problem minimum and that two README sections exist (Section 5.2); it never inspects the calibration ensembles or evidence packages that Stages C and D require. Nothing therefore flagged that the calibration ensembles cited for the Stage C labels of 01, 02, 07, 14 and 17 (generated 2026-04-10 to 2026-04-12) predate the replacement of those problems' VQE hardware kernels by QPE kernels (2026-04-18), that their resource estimates still target the VQE routines, or that three Stage C problems lack valid ensemble statistics: 16 has three runs, and 14 and 18 report means, deviations and intervals of zero because their tuple- and list-valued outputs were never parsed. Checking the artifacts each stage requires, and binding each artifact to a hash of the kernel it describes, would turn these into failing checks.
...
- Re-run the emulator, noise and calibration studies for the five QPE kernels, and point their resource estimates at the QPE routines (Section 8, limitation 8)
- Bind each evidence artifact to a hash of the kernel it describes, and make the CI checks required for merging, so that stale evidence fails a build
...
|---|---|---|---|---|---|
| 01 | Hubbard Model | VQE (kernel now QPE) | 2-3 | C | Active |
| 02 | Catalysis | VQE (kernel now QPE) | 2-3 | C | Active |
...
| 07 | Drug Discovery | VQE (kernel now QPE) | 2-3 | C | Active |
...
| 14 | Materials | VQE (kernel now QPE) | 2-3 | C | Active |
| 15 | Database Search | Grover | 4-12 | D | Archived |
| 16 | Error Correction | QEC | 5 | C | Active |
| 17 | Nuclear Physics | VQE (kernel now QPE) | 2-3 | C | Active |
