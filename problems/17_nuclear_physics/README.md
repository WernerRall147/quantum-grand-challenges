...
- **Current gate**: **Stage C complete** (hardware-aware validation evidence in place: QPE deuteron kernel runnable, calibration ensemble across runs, backend assumptions documented, dual-model estimator profile, OpenQASM export, Azure smoke validation).
- **Next gate target**: **Stage D** (advantage evidence package: fairness review against shell-model/lattice-QCD baselines for >20-nucleon systems, residual risks for naturally-quantum nuclear forces, claim category locked).
...
- Cross-platform OpenQASM export: `estimates/vqe_deuteron.qasm`.
