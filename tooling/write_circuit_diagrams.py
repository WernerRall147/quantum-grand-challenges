...
SUPERSEDED. circuit.txt is now generated by trace_circuits.py from each
problem's HardwareKernel.qs, which is what Azure Quantum actually receives.
Running this script overwrites those files with these hand-written diagrams,
several of which still describe the VQE ansatz that five problems replaced
with QPE. Re-run trace_circuits.py afterwards if you run this at all.
"""
