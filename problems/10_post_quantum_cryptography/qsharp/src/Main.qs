...
function ClassicalSearchCost(keyspaceBits : Int) : Double {
    // Use 2.0^bits to avoid Int overflow for large keyspaces (128, 256)
    let base = new Complex { Real = 2.0, Imag = 0.0 };
    let power = new Complex { Real = IntAsDouble(keyspaceBits), Imag = 0.0 };
    return PowC(base, power).Real / 2.0;
}
