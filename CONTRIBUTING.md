...
## Dependency map and cleanup

This repo keeps a deterministic dependency + reachability map under `docs/depgraph/`,
generated by `tooling/depgraph/build_graph.py` (standard library only).

- After adding, moving, or removing any `.py`, `.qs`, `.ts`, or `.tsx` file (or a
  `Makefile`, a workflow, the `Dockerfile`, or a `website/package.json` script), run
  `python tooling/depgraph/build_graph.py` and commit the updated `docs/depgraph/*.json`.
  The Dependency Graph Drift Check CI job fails if the committed map is stale.
- Before removing code, check `docs/depgraph/cleanup-candidates.json` and the danger list in
  `docs/initiatives/repo-cleanup.md`. Do not delete anything reachable from an entry point,
  matching the danger list, or whose removal is not proven safe by a green build + tests.
